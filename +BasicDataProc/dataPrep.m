...
%   notch filter, remove dc offset, common average rereference, bandpass
%   filters


%[filtData] = Analysis.NPTL.dataPrep(data); %if the filters are not made
%already
%[filtData] = Analysis.NPTL.dataPrep(data, 'bandfilterA', bandfilter); %if the filters are made
...
[varargin, commonAverage] = util.argkeyval('commonAverage',varargin, true); % option to do a common average rereference
...
[varargin, DoBandFilterBroad] = util.argkeyval('DoBandFilterBroad', varargin, true);  %if you want to also run basic band filters (different than the 2 hz filter method below) in classic frequencies (delta 1-4 theta 4-8 alpha 8-13 beta 13-30 gamma 30-50 high gamma 50 to 200)
[varargin, IIR] = util.argkeyval('IIR',varargin, 0);  % IIR vs FIR, UCSF uses FIR for phase stuff
...
if itiProc
    ...
    dataTemp=repmat(dataMean, 1, size(dataTemp,2), 1);
end
    ...
    if dBPower
        ...
        if DoBandFilterBroad && IIR
            ...
            for ii=1:size(dataTempClassicBandT,3)
                [dataTempClassicBandT(:,:,ii), tplotC]=Analysis.BasicDataProc.convSmooth(dataTempClassicBandT(:,:,ii), 30, fs);
            end
