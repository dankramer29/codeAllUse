...
debug = Debug.Debugger('analysis_20180507');
...
experiments = hst.getExperiments(taskname);
for ee=1:size(experiments,1)
    ...
    for tt=1:length(taskfiles)
        ...
        for gg=1:map.NumGrids
            ...
            tile_prc = [25 75];
            multiple_relaxed = 3;
            samples_relaxed = ceil(0.03*size(dtfreq_outlier,1));
            multiple_harsh = 5;
        ...
        fig = figure('Position',[100 100 1800 600],'PaperPositionMode','auto');
        for cc=1:blc.ChannelCount
