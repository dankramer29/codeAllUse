...
tt=specgramcBM5.SoftTouch.t;
ff=specgramcBM5.SoftTouch.f;
...
figure
subplot(8, 2,[1 4])
imagesc(tt, ff, specgramcBM5.DeepTouch.MG48); axis xy
...
subplot(8, 2, 5)
imagesc(tt, ff, elec_pBM5.DeepTouch{1}.clust_p'); axis xy
ax.FontName='Arial';
xlim([-0.1 1.6]); %only show the plot buffer pre and buffer post
%title([' Channel ', blc.ChannelInfo(ch2plot_order(ii)).Label])
colormap(inferno(100))
...
subplot(8, 2,[7 10])
imagesc(tt, ff, specgramcBM55.DeepTouch.MG48); axis xy
...
subplot(8, 2, 11)
imagesc(tt, ff, elec_pBM55.DeepTouch{1}.clust_p'); axis xy
ax.FontName='Arial';
xlim([-0.1 1.6]); %only show the plot buffer pre and buffer post
%title([' Channel ', blc.ChannelInfo(ch2plot_order(ii)).Label])
colormap(inferno(100))
...
alpham=mean(continuousPowerdB.bm(48).DeepTouch.Alpha,2);
alphasd=std(continuousPowerdB.bm(48).DeepTouch.Alpha,[], 2);
betam=mean(continuousPowerdB.bm(48).DeepTouch.Beta,2);
betasd=std(continuousPowerdB.bm(48).DeepTouch.Beta,[],2);
hold on
H=shadedErrorBar(tt2000, real(alpham), real(alphasd), 'lineprops', {'-r', 'markerfacecolor', C(1,:)});
...
H.edge(1).Color=C(1,:);
H.edge(2).Color=C(1,:);
H=shadedErrorBar(tt2000, real(betam), real(betasd), 'lineprops', {'-r', 'markerfacecolor', C(1,:)});
H.mainLine.LineWidth=4; 
H.patch.FaceColor=C(12,:);
H.patch.EdgeColor=C(12,:);
H.mainLine.Color=C(12,:);
H.edge(1).Color=C(12,:);
H.edge(2).Color=C(12,:);
xlim([-0.1 1.6])
...
for ii=1:length(nn)
    ...
    if ii==1
        set(p1, 'FaceColor', C(2,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p2, 'FaceColor', C(3,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p3, 'FaceColor', C(4,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        title('Alpha')
        
    elseif ii==2
        set(p1, 'FaceColor', C(7,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p2, 'FaceColor', C(8,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p3, 'FaceColor', C(9,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        title('Beta')
        
    elseif ii==3
        set(p1, 'FaceColor', C(29,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p2, 'FaceColor', C(28,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p3, 'FaceColor', C(27,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        title('Gamma')
     
    elseif ii==4
        set(p1, 'FaceColor', C(36,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p2, 'FaceColor', C(35,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        set(p3, 'FaceColor', C(34,:), 'EdgeColor', 'k', 'BarWidth', wdth);
        title('High Gamma')
...
for idxbands=2:5
    for idxtouch=1:3
        ...
        if isempty(meantouchZ.(nmes{1}).(touch{idxtouch}).(bands{idxbands})(:,1))
            ...
            p1=plot(tt2000, meantouchZ.(nmes{1}).(touch{idxtouch}).(bands{idxbands})(:,1), 'color', C(clrs(clridx),:), 'LineWidth', 2);         
        ...
        if isempty(meantouchZ.(nmes{2}).(touch{idxtouch}).(bands{idxbands})(:,1))
            ...
            p2=plot(tt256, meantouchZ.(nmes{2}).(touch{idxtouch}).(bands{idxbands})(:,1), 'color', C(clrs(clridx+1),:), 'LineWidth', 2);
        ...
        if isempty(meantouchZ.(nmes{3}).(touch{idxtouch}).(bands{idxbands})(:,1))
            ...
            p3=plot(tt2000, meantouchZ.(nmes{3}).(touch{idxtouch}).(bands{idxbands})(:,1), 'color', C(clrs(clridx+2),:), 'LineWidth', 2);
        ...
        if isempty(meantouchZ.(nmes{4}).(touch{idxtouch}).(bands{idxbands})(:,1))
            ...
            p4=plot(tt256, meantouchZ.(nmes{4}).(touch{idxtouch}).(bands{idxbands})(:,1), 'color', C(clrs(clridx+3),:), 'LineWidth', 2);
        ...
        if idxtouch==3
            if idxbands~=5
                legend([p1 p2 p3 p4], {'S01', 'S02', 'S03', 'S04'}, 'Location', 'bestoutside', 'FontSize', 16);
            else
                legend([p1 p2 p4], {'S01', 'S02',  'S04'}, 'Location', 'bestoutside');
            end
        end
...
%%making a plot for the bar graph of quantities
N=36;
C=linspecer(N);
clrs=[1,3, 5, 7; 9, 11, 13 15; 22, 24, 26, 28; 32, 34, 35, 36]';
lbl={'Any Band'; 'Alpha'; 'Beta'; 'Gamma'; 'High Gamma'};
lblnm={ 'S01'; 'S02'; 'S03'; 'S04'};

lbl=categorical(lblnm);
figure
set(gca,'FontSize', 22)
