function [sig_vals] = shuffle_comparison(PowerArray, Targets, FreqBins, NumSamp, varargin)
    
    
     [varargin, CompMethod, ~, ~] = util.argkeyval('CompMethod', varargin, 'perc_diff');
...
%     rsampleidx = logical(gpuArray.zeros(listlength, 1));
    ...
    %------------Method 1-------------------
    if strcmp(CompMethod, 't-test')
        
        mean_diff_low = zeros(tb, ch, fb);
        mean_diff_high = zeros(tb, ch, fb);
        sig_val_low = zeros(tb, ch, fb);
        sig_val_high = zeros(tb, ch, fb);
    ...
    %             sample_t_val(:, :, i) = md ./ sqrt((sv./num_R) + (sv./num_L));
            ...
            sig_val_low(:, :, f) = stv_sort(:, :, low_i);
            sig_val_high(:, :, f) = stv_sort(:, :, high_i);
        end %end for t-test freq band
        sig_vals{1, 1} = sig_val_low;
        sig_vals{1, 2} = sig_val_high;
    ...
    elseif strcmp(CompMethod, 'perc_diff')
        sig_vals = cell(1, 2);
        sig_val_low = zeros(tb, ch, fb);
        sig_val_high = zeros(tb, ch, fb);
        ...
        for f = 1:fb
            ...
            fb_array = permute(fb_array, [1 3 2 4]);
            %sample_var = gpuArray.zeros(tb, ch, NumSamp);
            sample_p_diffs = gpuArray.zeros(tb, ch, NumSamp);
            ...
            for i = 1:NumSamp
                ...
                sample_p_diffs(:, :, i) = (rsample_mean - lsample_mean) ./ (rsample_mean + lsample_mean);
            end %end for perc_diff num_samp
            toc
            sort_pdiff = gather(sort(sample_p_diffs, 3));
            sig_val_low(:, :, f) = sort_pdiff(:, :, low_i);
            sig_val_high(:, :, f) = sort_pdiff(:, :, high_i);
        end %end for perc_diff freq band
        sig_vals{1, 1} = sig_val_low;
        sig_vals{1, 2} = sig_val_high;
    else
        fprintf('No code for comparison method %s available\n', CompMethod)
    end %end if CompMethod ==
end
