function packets = getProcwinNSxPackets(ns,procwin,varargin)
% GETPROCWINNSXPACKETS Identify recording packets associated with procwins
%
% SUMMARY:
...
numArrays = length(ns);
if iscell(procwin)
    assert(length(procwin)==length(ns),'Must provide same number of procwin cells as ns cells');
else
...
for kk=1:numArrays
    ...
    st = procwin{kk}(:,1);
    len = procwin{kk}(:,2);
    [~,startPackets,~,endPackets] = arrayfun(@(x)fn(ns{kk},st(x),len(x),ref),1:numTrials,'UniformOutput',false);
