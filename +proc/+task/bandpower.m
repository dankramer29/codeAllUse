function [pwr,relt,freqband,featdef] = bandpower(task,params,debugger,varargin)
% TRIALBANDPOWER Generate band power for whole trials
%
%   [PWR,RELT,FREQBAND,FEATDEF] = TRIALBANDPOWER(TASK,PARAMS,DEBUGGER)
%   For the task represented by FRAMEWORKTASK object TASK and parameters
%   defined by PARAMETERS.DYNAMIC object PARAMS, calculate band power
%   PWR for the indicated trials, and the time and frequency indices RELT
%   and FREQBAND.  Also return FEATDEF which describes the source of each
%   feature. Provide a DEBUG.DEBUGGER object in DEBUGGER.
%
%   [PWR,RELT,FREQBAND,FEATDEF] = TRIALBANDPOWER(...,TRIALIDX)
%   Specify which trials to process.  TRIALIDX can be defined logically or
%   numerically indexed.
%
%   [PWR,RELT,FREQBAND,FEATDEF] = TRIALBANDPOWER(...,'UNIFORMOUTPUT',FLAG)
...
% process inputs
[trialidx,~,varargin] = proc.helper.trialIndices(varargin,task,debugger);
assert(isempty(varargin),'Unexpected inputs');

% check whether data is cached, and if so load it
set_require = list(params,'dt','tm','spc','lfp');
set_except = {'dt.cacheread','dt.cachewrite','dt.mintrials','spc.smoothparams'};
[cached,valid] = cache.query(tag,params,debugger,'require',set_require,'except',set_except);
if cached && valid && params.dt.cacheread
    [pwr,relt,freqband,featdef] = cache.load(tag,debugger);
...
% get the NSx objects
ns = proc.helper.getNSx(task,params,debugger);
...
if ~isempty(params.tm.baseline)
    ...
    [baseline_pwr,baseline_relt,baseline_len] = getTrialData(baseline,ns,lag,task,params,debugger);
end
analysis = params.tm.analysis;
[pwr,relt,freqband,len,featdef] = getTrialData(analysis,ns,lag,task,params,debugger);
...
if params.spc.smooth
    debugger.log(sprintf('Smoothing the data: %s',util.any2str(params.spc.smoothmethod)),'info');
    params.spc.smoothparams.period = params.spc.movingwin(2);
    if ~isempty(baseline_pwr)
        baseline_pwr = cellfun(@(x)proc.smooth(x,params.spc.smoothmethod,params.spc.smoothparams),baseline_pwr,'UniformOutput',false);
    end
    pwr = cellfun(@(x)proc.smooth(x,params.spc.smoothmethod,params.spc.smoothparams),pwr,'UniformOutput',false);
end
...
pwr = cellfun(@(x,y)x(y,:,:),pwr(:),analysis_idx_keep(:),'UniformOutput',false);
...
if params.spc.timeavg
    ...
    pwr = cellfun(@(x)squeeze(mean(x,1)),pwr,'UniformOutput',false); % average over time
end
...
if params.dt.cachewrite
    cache.save(tag,params,debugger,pwr,relt,freqband,featdef);
end
...
function [pwr,relt,freqband,len,featdef] = getTrialData(window,ns,lag,task,params,debugger)
% GETTRIALDATA Extract trials from full bin dataset
%
%   [PWR,RELT,LEN] = GETTRIALDATA(WINDOW,LENFCN,NS,LAG,TASK,PARAMS,DEBUGGER)
...
[pwr,relt,freqband,featdef] = proc.blackrock.bandpower(ns,...
    'movingwin',params.spc.movingwin,...
    'freqband',params.spc.freqbands,...
    'procwin',[tm(:) len(:)],...
    'lag',lag,...
    'cache',cachearg,...
    'gpu',gpuarg,...
...
function [pwr,relt] = postprocess(pwr,relt,trialidx,flagUniformOutput)
...
if flagUniformOutput
    fs = 1/median(unique(diff(relt{1})));
    p = 10^(floor(log10(fs))+2); % precision is next-next higher power of 10 than the sampling frequency
    [pwr,relt] = proc.helper.createUniformOutput(pwr,relt,'precision',p);
end