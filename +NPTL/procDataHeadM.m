...
for ii=500:length(rState)-trial %start 500ms in, should be all garbage but some weird number things and end before length of trial
    ...
    elseif rState(ii)==4 && rState(ii-1)==3 %3 is move, 4 is hover
        ...
        if sum(rState(ii:ii+epoch)==5)>0 %so if the trial ends and it wasn't a false endpoint
            %500 seconds on either side of stop
            rEndPoint(idxTHe,1)=ii;
            ...
            tempSt=find(tSpec>=ii-epoch-bufferT); tempEnd=tempSt(1)+tSpecEpoch;
            endPointDataSpAllCh{idxTHe}=DataAll.dataSpec.data(:, tempSt(1):tempEnd(end), :);
            tempSt=find(tcB>=ii-epoch); tempEnd=tempSt(1)+tCbEpoch;
            endPointCBAll{idxTHe}=CBPowerAll(tempSt(1):tempEnd(end), :, :);
            %transHoldData{idxTHe}=data(transHoldFCT(idxTHe, 1)+((transHoldPre-epoch-bufferT)*30):transHoldFCT(idxTHe, end)+((bufferT)*30),:); %add the buffer for processing and plotting
            tempSt=find(tm>=ii-epoch); tempEnd=tempSt(1)+tMEpoch;
            endPointSmooth{idxTHe}=spkRateSmooth(tempSt(1):tempEnd(end),:);
            %get spikes averaged across the channels, for each trial
            tempSt=[]; tempEnd=[];
            endPointDataSpChMean(:,:,idxTHe)=nanmean((transHoldDataSpAllCh{idxTHe}),3); %mean across all channels
            endPointBandDataChMean(:,:,idxTHe)=nanmean(transHoldCBAll{idxTHe},3); %mean across all channels
            endPointSmoothChMean(:,idxTHe)=nanmean(transHoldSmooth{idxTHe},2);   %mean across all channels
            %%find maxima and minima of the spikes and band data for each trial,
            %%this is too compare within each trial.
            endPointBandPeaksChMean(:,:,idxTHe)=Analysis.BasicDataProc.specChange(endPointBandDataChMean(downSample*bufferT/1000+1:end-downSample*bufferT/1000,:,idxTHe)); %remove the buffered ends for this analysis.
            endPointpkPeaksChMean(:,idxTHe)=Analysis.BasicDataProc.specChange(endPointSmoothChMean(:,idxTHe));
            for jj=1:length(ChSpk)
                bychEndPointSpk{jj}(:, idxTHe)=endPointSmooth{idxTHe}(:, ChSpk(jj)); %cells by channel, with trials by spike averages
                bychEndPointData{jj}(:,:,idxTHe)=endPointDataSpAllCh{idxTHe}(:,:,ChSpk(jj)); %load a 3d matrix for time x freq x trial by channel for spectral data
                bychEndPointBandData{jj}(:,:,idxTHe)=endPointCBAll{idxTHe}(:,:,ChSpk(jj));
            end
            
            idxTHe=idxTHe+1;
        end
        %% Transition to move state
    elseif rState(ii)==5 && rState(ii-1)==4 %3 is move, 4 is hover
        rTransMoveEnd(idxTM,1)=ii;
        transMoveCP{idxTM}=rCP(:,ii+transMove:ii+tranMove+epoch);
        transMoveSpk{idxTM}=spk(ii+transMove:ii+tranMove+epoch, :); %take all the channels at once
        transMoveFCT(idxTM, :)=rFCT(1,ii);
        tempSt=find(tSpec>=ii+transMovePre-bufferT); tempEnd=tempSt(1)+tSpecEpoch;
        transMoveDataSpAllCh{idxTM}=DataAll.dataSpec.data(:, tempSt(1):tempEnd(end), :);
        tempSt=find(tcB>=ii+transMovePre); tempEnd=tempSt(1)+tCbEpoch;
        ...
        tempSt=find(tm>=ii+transMovePre); tempEnd=tempSt(1)+tMEpoch;
        ...
        idxTM=idxTM+1;
        %% Prep state
    elseif rState(ii)==2 && rState(ii-1)==1 %2 is target appears, 1 is end of last trial, at center
        rPrepEnd(idxPr,1)=ii;
    ...
    elseif rState(ii)==3 && rState(ii-1)==2 %2 is target appears, 1 is end of last trial, at center
